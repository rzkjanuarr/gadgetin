...
class AbsenModel extends CI_Model{
	...
	public function lihat_absen(){
		$this->db->select('*');
		$this->db->from('gadgetin_absen');
		$this->db->join('gadgetin_karyawan', 'gadgetin_karyawan.karyawan_id = gadgetin_absen.absen_karyawan_id');
	...
	public function tambah_absen($data){
		$this->db->insert('gadgetin_absen', $data);
	...
	public function update_absen($id,$data){
		$this->db->where('absen_id', $id);
		$this->db->update('gadgetin_absen', $data);
		return $this->db->affected_rows();
	}
}
